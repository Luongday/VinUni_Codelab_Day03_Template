...
class ReActAgent:
    ...
    def plan_and_execute_step(self, user_input: str, iteration: int) -> Tuple[str, bool]:
        ...
        if needs_flight and iteration == 1:
            ...
            flight_obs = next(
                (t["observation"] for t in self.trace if t.get("action", {}).get("name") == "get_flight_info"), [])
            weather_obs = next(
                (t["observation"] for t in self.trace if t.get("action", {}).get("name") == "get_weather_forecast"), {})
